...
public class FileStorageService {
  ...
  public FileStorageService(@Value("${file.upload-dir}") String uploadDir, QuizRepository quizRepository) {
  ...
  public void storeFile(MultipartFile file, int quizId) {
    ...
    try {
      ...
      Quiz quiz = quizRepository.findById((long) quizId).orElseThrow(() -> new RuntimeException("Quiz not found with id " + quizId));
